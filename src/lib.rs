...
//! ## Why?
//!
//! I didn't want to use a fixed cache size when I expect that most data will
//! not be fetched twice, and that most of the time the number of items
//! benefitting from caching will be small. Good use cases: parsing large data
//! structures that frequently cross-reference the same data chunk, reading a
//! set of dictionary-compressed files where there are several different
//! dictionaries, reading many files that all refer to shared parser profiles
//! (eg. color profiles in images), etc.
//!
...
/// A cache that will only hold onto items that have been requested more than
/// once in recent memory. Single-use items are not held at all. Once an item is
/// requested twice, it is cached until all memory of seeing a request has
/// expired. The length of the memory is adjustable, and must be set at
/// initialization.
